...
function renderSearchOptions() {
    if (searchEnabled) {
        ...
        searchField.appendChild(createOption("Search by...", NO_SEARCH_OPTION_SELECTED));
...
function renderSortOptions() {
    if (sortEnabled) {
        ...
        columnSettings.forEach(col => {
            if (col.enabled) {
                const option = createOption(col.displayName, col.columnIndex);
...
function populateTableHead(cols) {
    ...
    tr.appendChild(th);
    cols.forEach(col => {
        const columnSetting = columnSettings.find(setting => setting.columnKey === col.id);
...
function renderData(json) {
    ...
    if (thead && tbody) {
        ...
        if (sortFieldIdx !== NO_SORT_OPTION_SELECTED) sort(json.table.rows, sortFieldIdx);
...
export function checkboxDropdownChangeListener(target) {
    if (target && target.matches(".form-check-input")) {
        const columnSetting = columnSettings.find(f => f.columnKey === target.value);
        if (columnSetting) {
